...
select m.name + N' ' + c.City as Name, '1234', c.City as City, case when c.Region = 'Strumicki' then 'Stipski' else c.Region end as Region, 1 as IsActive
from @Market m
CROSS APPLY @City c

update c set AccountNumber = id + cast(233255 * rand(id*7) as int),
Size = case when id%3 = 0 then 'Small' when id % 3 = 1 then 'Medium' else 'Large' end, 
...
INSERT INTO dbo.Products (Name, Code, Price)
VALUES('Crunchy' , '1111', 100),('Granola', '2222', 200),('Regular/soft', '3333', 300),('Gluten Free', '4444', 100),('Multigrain', '5555', 100),('Take away', '6666', 200),('Cornflakes', '7777', 100),('Cereals', '8888', 100),
('Protein Bar', '9999', 100),('Raw bars', 'AAAA', 100),('No Sugar Added', 'SASA', 150),('Multigrain New', 'DDDD', 100),
('Gluten Free New', 'FRDS', 150),('Kids Bar', 'AAAV', 100),('Fruit, Nuts & Seeds', 'SSSS', 100)
...
select * from dbo.Employees where FirstName = 'Goran'
--find all Employees with LastName starting with S
select * from dbo.Employees where Lastname like 'S%'
--find all employees that where hired on January
select * from dbo.Employees where HireDate >= '1998.01.01' and HireDate <= '1998.01.31'
--find all employees that where hired on 1st of january
select * from dbo.Employees where HireDate = '1998.01.01'
--find all employees with lastname starts with 'P' and hired in january 1988
select * from dbo.Employees where Lastname like 'P%' and HireDate >= '1998.01.01' and HireDate <= '1998.01.31'
--find all Male employees
select * from dbo.Employees where Gender = 'M'
--find all employees hired in January/1998 and March/1998
select * from dbo.Employees where (HireDate >= '1998.01.01' and HireDate <= '1998.01.31') or (HireDate >= '1998.03.01' and HireDate <= '1998.03.31')